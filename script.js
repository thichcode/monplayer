class MonPlayer {
    ...
    async loadPlaylist() {
        if (!this.urlInput) {
            ...
            this.showToast('Không thể tải playlist: Đầu vào URL không tồn tại', 'error');
            return;
        }
        ...
        try {
            ...
            if (this.validateIPTVData(data)) {
                ...
                this.showToast(`Tải thành công! Đã phát hiện ${this.channels.length} kênh`, 'success');
            } else {
    ...
    validateIPTVData(data) {
        ...
        if (data.epg && Array.isArray(data.channels)) {
            ...
            return data.list.length > 0 && data.list.every(channel => 
    ...
    processChannels(data) {
        ...
        if (data.epg && Array.isArray(data.channels)) {
            ...
            channels = data.list.map(channel => ({
                id: channel.id || `channel-${Math.random().toString(36).substr(2, 9)}`,
                name: channel.title || channel.name || 'Kênh không tên',
                url: channel.url || channel.stream_url,
                logo: channel.logo || channel.icon,
                category: channel.group || channel.category || 'Khác',
                description: channel.description || '',
                ...
                if (Array.isArray(group.channels)) {
                    const groupChannels = group.channels.map(channel => {
                        ...
                        if (channel.sources && channel.sources.length > 0) {
                            ...
                            if (firstSource.contents && firstSource.contents.length > 0) {
                                const firstContent = firstSource.contents[0];
                                if (firstContent.streams && firstContent.streams.length > 0) {
                                    const firstStream = firstContent.streams[0];
                                    if (firstStream.remote_data && firstStream.remote_data.url) {
                                        streamUrl = firstStream.remote_data.url;
                                    }
                                }
                            }
                        }
                        ...
                        return {
                            ...
                            name: channel.name || channel.title || 'Kênh không tên',
                            url: streamUrl,
                            logo: channel.image ? channel.image.url : null,
                            category: channel.group || group.name || 'Khác',
                            description: channel.description || '',
                            language: channel.language,
                            country: channel.country
                        };
                    });
                    channels = [...channels, ...groupChannels];
                }
                
                if (group.groups && Array.isArray(group.groups)) {
                    group.groups.forEach(subGroup => {
                        if (Array.isArray(subGroup.channels)) {
                            const subGroupChannels = subGroup.channels.map(channel => {
                                let streamUrl = null;
                                ...
                                return {
                                    ...
                                    url: streamUrl,
                                    logo: channel.image ? channel.image.url : null,
                                    category: channel.group || subGroup.name || 'Khác',
                                    description: channel.description || '',
                                    language: channel.language,
                                    country: channel.country
                                };
                            });
                            channels = [...channels, ...subGroupChannels];
                        }
                    });
                }
            });
    ...
    async fetchActualStreamUrl(streamUrl) {
        try {
            const response = await fetch(`/api/stream?url=${encodeURIComponent(streamUrl)}`);
            ...
            if (data.stream_links && data.stream_links.length > 0) {
                // Find the default HD M3U8 stream
                const defaultStream = data.stream_links.find(link => 
                    link.type === 'hls' && link.default === true
                );
                ...
                // Fallback to first M3U8 stream
                const m3u8Stream = data.stream_links.find(link => 
                    link.type === 'hls'
                );
        ...
        if (Hls.isSupported()) {
            ...
            video.addEventListener('loadedmetadata', () => {
                video.play().catch(error => {
                    this.showToast('Không thể tự động phát, vui lòng nhấn nút phát', 'warning');
                });
            });
        } else {
